...
public class TokensRealmSource implements TokenLocalSource {
    ...
    private void setTokenUpdateTime(Realm realm, Token token, int assetCount)
    {
        ...
        if (realmToken != null)
        {
            ...
            else if (!realmToken.isVisibilityChanged() && assetCount == 0 && !token.tokenInfo.address.toLowerCase().equals(Token.CLOUCH_TOKEN.toLowerCase()))
            {
    ...
    public boolean updateTokenBalance(Wallet wallet, Token token, BigDecimal balance, List<BigInteger> balanceArray)
    {
        ...
        try (Realm realm = realmManager.getRealmInstance(wallet))
        {
            ...
            if (realmToken != null)
            {
                ...
                if (!realmToken.isVisibilityChanged() && realmToken.isEnabled() && newBalance != null && newBalance.equals("0") && !token.tokenInfo.address.toLowerCase().equals(Token.CLOUCH_TOKEN.toLowerCase()))
                {
    ...
    private void saveToken(Realm realm, Token token) throws RealmException
    {
        ...
        if (realmToken == null)
        {
            if (BuildConfig.DEBUG) Log.d(TAG, "Save New Token: " + token.getFullName() + " :" + token.tokenInfo.address + " : " + token.balance.toString() + " - " + token.tokenInfo.isEnabled );
                ...
                && !realmToken.getEnabled() && !realmToken.isVisibilityChanged())
        {
            ...
            realmToken.setEnabled(true);
                ...
                && realmToken.getEnabled() && !realmToken.isVisibilityChanged() && !token.tokenInfo.address.toLowerCase().equals(Token.CLOUCH_TOKEN.toLowerCase()))
        {
            token.tokenInfo.isEnabled = false;
            realmToken.setEnabled(false);
        }
