...
import static com.alphawallet.app.entity.WalletPage.CLOUCH;
        ...
        FragmentMessenger, Runnable, SignAuthenticationCallback, LifecycleObserver
{
    ...
    private final Fragment clouchBrowserFragment;
    ...
    public HomeActivity()
    {
        ...
        clouchBrowserFragment = new ClouchBrowserFragment();
                ...
                this, isOpen -> {
                    {
                        ...
                        if (isOpen) {
                            ...
                            if (requiresDappBrowserResize)
                                ((DappBrowserFragment) dappBrowserFragment).softKeyboardVisible();
                        } else {
                            setNavBarVisibility(View.VISIBLE);
                            if (requiresDappBrowserResize)
                                ((DappBrowserFragment) dappBrowserFragment).softKeyboardGone();
                        }
                    }

                    {
                        boolean requiresDappBrowserResize = !viewModel.fullScreenSelected() && viewPager.getCurrentItem() == CLOUCH.ordinal();
                        if (isOpen) {
                            setNavBarVisibility(View.GONE);
                            if (requiresDappBrowserResize)
                                ((ClouchBrowserFragment) clouchBrowserFragment).softKeyboardVisible();
                        } else {
                            setNavBarVisibility(View.VISIBLE);
                            if (requiresDappBrowserResize)
                                ((ClouchBrowserFragment) clouchBrowserFragment).softKeyboardGone();
                        }
                    }
    ...
    public boolean onBottomNavigationItemSelected(WalletPage index)
    {
        switch (index)
        {
            case CLOUCH:
            {
                if (getSelectedItem() == CLOUCH)
                {
                    //((DappBrowserFragment) clouchBrowserFragment).homePressed();
                }
                else
                {
                    showPage(CLOUCH);
    ...
    private void showPage(WalletPage page)
    {
        ...
        switch (page)
        {
            ...
            {
                hideToolbar();
                viewPager.setCurrentItem(CLOUCH.ordinal());
                setTitle(getString(R.string.toolbar_header_browser));
                selectNavigationItem(CLOUCH);
        ...
        switch (requestCode)
        {
            case DappBrowserFragment.REQUEST_CAMERA_ACCESS:
                if (viewPager.getCurrentItem() == DAPP_BROWSER.ordinal()) {
                    ...
                    ((ClouchBrowserFragment) clouchBrowserFragment).gotFileAccess(permissions, grantResults);
                }
                break;
    ...
    public void onBackPressed()
    {
        ...
        } else if (viewPager.getCurrentItem() == CLOUCH.ordinal()) {
            ((ClouchBrowserFragment)clouchBrowserFragment).backPressed();
        }
